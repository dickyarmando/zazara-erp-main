    ...
    <div class="col-md-offset-1 col-md-12">
        <div class="panel">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-8 col-xs-12 text-right">
                        <div class="d-md-flex justify-content-end">
                            <a href="{{ url('masters/users/create') }}" class="btn btn-primary btn-sm"><i
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="sort" wire:click="sortOrder('users.name')">Full Name {!! $sortLink !!}
                            </th>
                            <th class="sort" wire:click="sortOrder('users.username')">Username {!! $sortLink !!}
                            </th>
                            <th class="sort" wire:click="sortOrder('users.email')">Email {!! $sortLink !!}</th>
                            <th class="sort" wire:click="sortOrder('users.phone')">Phone {!! $sortLink !!}</th>
                            <th class="sort" wire:click="sortOrder('prm_roles.name')">Role {!! $sortLink !!}
                            </th>
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr>
                                ...
                                <td class="border-start">{{ $user->username }}</td>
                                <td class="border-start">{{ $user->email }}</td>
                                <td class="border-start">{{ $user->phone }}</td>
                                <td class="border-start">{{ $user->role_name }}</td>
                                <td class="border-start text-center">
                                    <button type="button" wire:click="edit('{{ $user->id }}')"
                                        class="btn btn-xs btn-secondary me-2" title="Edit User"><span
            ...
            <div class="panel-footer">
                {{ $users->links('admin.custom-pagination-new') }}
            </div>
        </div>
    </div>

    {{-- Delete --}}
    <div wire:ignore.self class="modal fade" id="UserDeleteModal" tabindex="-1" ruangan="dialog">
